...
model='mistral-1'
source_folder = f'./{model}'

# Folder to save the renamed files
destination_folder = f'./new/{model}'
...
for root, dirs, files in os.walk(source_folder):
    for file in files:
        if file.endswith('.json'):
            ...
            cleaned_filename = clean_filename(file.replace('.json', ''))
            ...
            if not matching_row.empty:
                ...
                new_file_path = os.path.join(new_clip_folder, f'{new_id}.json')
