...
task_content=$(tr -d '\r' < "$task_folder/dimension_tasks/$dimension/$prompt_version.txt" | tr -d '\n')
...
find "$data_folder/$dataset/transcripts/$clip" -type f -name "*.txt" -print0 | while IFS= read -r -d '' transcript_file; do
    ...
    output_folder="prompts/$dataset/$dimension/$prompt_version/$clip/" # Updated output folder path
