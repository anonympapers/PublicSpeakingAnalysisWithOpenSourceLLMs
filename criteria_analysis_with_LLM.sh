...
prompt_files=($(find prompts/$dataset/$criteria_category/$criteria/$prompt_version/$clip/ -type f -name "*.txt"))
# echo $prompt_files
if [ ${#prompt_files[@]} -gt 0 ]; then
    ...
    for prompt_file in "${prompt_files[@]}"; do
        ...
        output_folder="$annotation_results_folder/model_raw_output/$dataset/$criteria_category/$criteria/$prompt_version/$model_name-$test_number/$clip/"
